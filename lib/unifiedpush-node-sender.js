...
/**
    ...
    @param {String} [options.simplePush] - simplePush version number
    @param {Number} [options.ttl] - the time to live in seconds. This value is supported by APNs and GCM Only
...
AeroGear.Sender.prototype.send = function( message, options, callback ) {
    ...
    var serverSettings, option, crit, key,
        url = urlParser.parse( this.getUrl() ),
        newMessage = {};

    for( option in options ) {
        if( option === "criteria" ) {
            for( crit in options.criteria ) {
                newMessage[ crit ] = options.criteria[ crit ];
            }
        } else if( option === "simplePush" ) {
            newMessage[ "simple-push" ] = options[option];
