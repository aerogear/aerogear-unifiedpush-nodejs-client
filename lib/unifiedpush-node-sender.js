...
function createServerSettings( url, applicationId, masterSecret ) {
    var serverSettings = {
...
/**
    ...
    @param {String} [message.title] - A short string describing the purpose of the notification.
    @param {String} [message.action] - The label of the action button
    @param {Array} [message.urlArgs] - an array of values that are paired with the placeholders inside the urlFormatString value of your website.json file. Safari Only
    @param {String} [message.titleLocKey] - The key to a title string in the Localizable.strings file for the current localization. iOS Only
    @param {Array} [message.titleLocArgs] - Variable string values to appear in place of the format specifiers in title-loc-key. iOS Only
...
AeroGear.Sender.prototype.send = function( message, options, callback ) {
    ...
    serverSettings = createServerSettings( url, this.getApplicationId(), this.getMasterSecret() );
